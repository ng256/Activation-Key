...
{
  internal abstract class InternalBaseEncoding : Encoding, IBaseEncoding
  {
    ...
    {
        return this.GetMaxByteCount(chars.GetMaxCount<char>(index, count));
    }

    public override int GetCharCount(byte[] bytes, int index, int count)
    {
        return this.GetMaxCharCount(bytes.GetMaxCount<byte>(index, count));
    }
    ...
    protected virtual int GetCharCount(
      ...
      int byteIndex)
    {
      int maxCharCount = this.GetMaxCharCount(bytes.GetMaxCount<byte>(byteIndex));
    ...
    protected virtual int GetByteCount(
      ...
      int charIndex)
    {
      int maxByteCount = this.GetMaxByteCount(chars.GetMaxCount<char>(charIndex));
